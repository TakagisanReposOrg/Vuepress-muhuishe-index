---
title: チーム構成
icon: fa-solid fa-user-group
---
```mindmap
root((木絵社))
   制作チーム
     絵画部
         (ぬりえグループ)
         (原画チーム)
         (絵コンテ)
         (モデリンググループ)
         (アニメーションチーム)
     脚本部
     アフレコ部
         (ボーカルグループ)
         (AIグループ)
     アニメ音響部
     翻訳部
     編集部
   監督する
   プロモーション&運営グループ
```
# 主要担当者表示カード

```card
title: 嘿嘿嘿 （担当者）
desc: 「高木さん」を愛する高校生として、高木界隈では無名のファンとして、全力でチームを引っ張っていきます！ 高木さんの物語は終わらない！
logo: https://i.ibb.co/9YjxMQX/jyxu0h.webp
link: https://blog.jerrydeng.online
```
```card
title: 小瑀 （監督）
desc: アニメが好きなオタク
logo: https://i.ibb.co/vYqP5MW/jyxa97.webp
link: https://space.bilibili.com/1229191353
```
```card
title: MuFeng086 （監督）
desc: 愛は残り、人生は続いていきます。
logo: https://i.ibb.co/g6Rkzmv/sm67xi.webp
link: https://mufeng086.com
```
```card
title: 致以杰 （特別指導）
desc: 私は多面的に自分を成長させることに重点を置いている人です、よく食べてよく休むことを忘れないでください〜
logo: https://i.ibb.co/mzzJL8z/lnp4df.webp
```
```card
title: 坐等<br>（アニメーション制作・ポストプロダクション部担当）
desc: 職業-美術。 ps、ai、pr、aeなどのクリエイティブソフトの使用が得意。 私は電子絵画もできます。
logo: https://i.ibb.co/kKCbVF8/jyxp7b.webp
link: https://space.bilibili.com/1121247935
```
```card
title: Rie （原画担当）
desc: こんにちは、皆さんこんにちは、QQ チャンネルの皆さんから「黒奴隷」の愛称で親しまれているリエと申します。今回のイベントの絵画部門の責任者をさせていただいております。私たちの作品を気に入っていただければ幸いです、今後ともご連絡ください。 . よろしくお願いします。
logo: https://i.ibb.co/tbXgDmg/k2hytn.webp
link: https://space.bilibili.com/15838604
```
```card
title: 阿巴Abba_<br>（絵コンテチーム担当＆特別指導）
desc: 二次創作や個人アニメなどを作っている普通の高校生。
logo: https://i.ibb.co/NyT7BLP/kbcnkv.webp
link: https://space.bilibili.com/1203140540
```
```card
title: Mrtank<br>（翻訳 & AIキャストグループ担当）
desc: 社会人初心者プログラマで技術オタクの私は、GTA高木を作るための「モジュール制作技術」に光を当てるなど、自分にとって変な「スキルポイント」を光らせるのが得意で、人から見たら恥ずかしがるような仕事です。 このプロジェクトでは翻訳とAI吹き替えを担当しています。
logo: https://i.ibb.co/QpD8kwj/j263m5.webp
link: https://space.bilibili.com/1791122
```
```card
title: 江小白 （アニメ音響監督）
desc: 皆さんこんにちは、江小白です。私は大学生です。音楽が大好きで、歌が得意です。高校2年生の頃から、音楽制作や後処理の専門的な知識に触れ始めました。 . 技術はまだ成熟しておらず、私も勉強中です。 皆さんと一緒に奇跡を起こせることを楽しみにしています！
logo: https://i.ibb.co/rvH1Hm9/jyy57u.webp
link: https://space.bilibili.com/1685876549 
```
```card
title: 66<br>(宣伝チーム＆チームアカウント運営グループリーダー) 
desc: 私は「高木さん」への愛を動画で表現したくてそれをしただけの人間で、それ以外に才能はなく、ただ「高木さん」の視聴者やファンに幸せを届けたいと願うだけです。
logo: https://i.ibb.co/SnGxRdM/tvdr0s.webp
```

